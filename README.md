...
## ⚙️ Maintainer Setup Notes

*(Internal reference — safe to delete this section from the live README once your setup is stable.)*

**1. Repository requirement**
This file must live in a repository named exactly `nikhildataops` (matching your GitHub username) for it to render on your profile page.

**2. GitHub Stats card** — powered by a self-hosted GitHub Action (`lowlighter/metrics`), not an external server.
    ...
    steps:
      - uses: lowlighter/metrics@latest
        with:
          ...
          footer: no
```
> `METRICS_TOKEN` must be a **classic** Personal Access Token (fine-grained tokens are not supported for this action's GraphQL calls) with `public_repo` scope, added under **Settings → Secrets and variables → Actions**.

**3. Contribution Snake** — powered by a self-hosted GitHub Action.
...
**4. Streak Stats & Contribution Graph** — externally hosted (no setup required), using currently maintained endpoints:
`streak-stats.demolab.com` and `github-readme-activity-graph.vercel.app`.

**5. Repository permissions** — both workflows require **Settings → Actions → General → Workflow permissions → Read and write permissions** to be enabled.

**6. To extend this dashboard further** — send over a portfolio site, additional social links, or specific pinned projects, and they can be added as a dedicated "Featured Projects" section.
